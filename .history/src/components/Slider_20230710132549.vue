    ...
    <div class="carousel">
        <div class="carousel-content">
            <div class="carousel-track" ref="track">
                <div class="carousel-item" v-for="product in this.productNumber" :key="product.id">
                    <img :src="product.product1.image" :alt="product.name">
                    <h3>{{ product.product1.name }}</h3>
                    <p>{{ product.product1.description }}</p>
                </div>
            </div>
...
export default {
    ...
    methods: {
        next() {
            this.currentIndex += 1;
            if (this.currentIndex >= this.productNumber) {
                this.currentIndex = 0;
            }
            this.moveCarousel();
        },
        prev() {
            ...
            if (this.currentIndex < 0) {
                this.currentIndex = this.productNumber - 1;
            }
            this.moveCarousel();
        },
        goTo(index) {
