...
export default {
    ...
    //     this.products = fetch('src/components/storage/products.json')
    //         .then(response => response.json())
    //         .then(data => {
    //             this.products = data;
    //         });
    //     console.log(typeof(products.product1.name))
    // },
    methods: {
        next() {
            this.currentIndex += 1;
            if (this.currentIndex >= this.nbOfProducts) {
                this.currentIndex = 0;
            }
            this.moveCarousel();
        },
        prev() {
            ...
            if (this.currentIndex < 0) {
                this.currentIndex = this.nbOfProducts - 1;
            }
            this.moveCarousel();
        },
        goTo(index) {
        ...
        getText(productNumber: number){
            return 'products.product'+String(productNumber)+'.name';
        }
    }
}
</script>
